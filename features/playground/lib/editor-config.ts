...
export const getEditorLanguage = (fileNameOrExtension: string): string => {
  const normalizedInput = (fileNameOrExtension || "").toLowerCase().trim();
  const extension = normalizedInput.replace(/^\./, "").split(".").pop() || "";
