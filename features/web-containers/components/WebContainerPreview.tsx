...
import { transformToWebContainerFormat } from "../utils/transformer";
...
const WebContainerPreview: React.FC<WebContainerPreviewProps> = ({
  ...
  const hasAttemptedSetupRef = useRef(false);
  ...
  useEffect(() => {
    if (forceResetup) {
      hasAttemptedSetupRef.current = false;
  ...
  useEffect(() => {
    async function setupContainer() {
      ...
      if (!instance || isSetupComplete || isSetupInProgress || hasAttemptedSetupRef.current) return;

      try {
        hasAttemptedSetupRef.current = true;
